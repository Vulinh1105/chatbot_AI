    ...
    if client.collection_exists(COLLECTION_NAME):
        ...
        if new_pairs:
            ...
            for start in range(
                ...
                store.add_documents(
                    [doc for doc, _ in batch],
                    ids=[
                        point_id
                        for _, point_id in batch
                    ],
                )

                print(
                    f"Uploaded "
                    f"{min(start + batch_size, len(new_pairs))}"
                    f"/{len(new_pairs)} chunks"
                )
